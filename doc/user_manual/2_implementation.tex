...
The inversion should go towards the background model if the signal to noise ratio decreases. If a user would like to test this property, it is not straightforward to do so by just lowering the the \kw{signal-to-noise-ratio} input in the modelfile, due to the $Var(S)/Var(D)$ factor above. Instead the fraction $Var(S)/Var(D)$ should be lowered. This can be done for example by increasing the $Var(D)$ by adding noise to the data, or lower $Var(S)$ by for example dampen the wavelet.
