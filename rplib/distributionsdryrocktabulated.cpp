...
: DistributionsDryRock()
{
  ...
  if (mineral_k->GetIsShared() == true)
    mineral_k_ = mineral_k;
  else
    mineral_k_ = mineral_k->Clone();
  ...
  tabulated_method_(dist.tabulated_method_)
{
  ...
  if(dist.mineral_k_->GetIsShared() == false)
    mineral_k_ = dist.mineral_k_->Clone();
  else
    mineral_k_ = dist.mineral_k_;
...
DistributionsDryRockTabulated::~DistributionsDryRockTabulated()
{
  ...
  if(density_->GetIsShared() == false)
    delete density_;
  if(mineral_k_->GetIsShared() == false)
    delete mineral_k_;
...
DistributionsDryRockTabulated::GenerateSample(const std::vector<double> & trend_params)
{
  ...
  std::vector<double> u2(2); //for porosity and mineral_k
                                         ...
                                         const std::vector<double> & trend_params)
{
  ...
  DryRock * dryrock = new DryRockTabulatedModulus(bulk_sample, shear_sample, density_sample,
                                                  total_porosity_sample, mineral_k_sample, u_final);
...
{
  if (mineral_k_->GetIsDistribution() == true || elastic1_->GetIsDistribution() == true || elastic2_->GetIsDistribution() == true ||
...
DistributionsDryRockTabulated::HasTrend() const
{
  ...
  std::vector<bool> mineral_k_trend         = mineral_k_->GetUseTrendCube();
  ...
  for(int i=0; i<2; i++) {
    if(elastic1_trend[i] == true || elastic2_trend[i] == true || density_trend[i] == true ||
       mineral_k_trend[i] == true || total_porosity_trend[i] == true)
      has_trend[i] = true;
  }
