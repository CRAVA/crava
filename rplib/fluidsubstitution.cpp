                                ...
                                double                             & rho_new) const
{
  if (InputOK(rock, fluid_old, saturation_old, fluid_new, saturation_new))
    ...
    DoGassmannReuss(rock, fluid_old, saturation_old, vp_old, vs_old, rho_old, 
                          fluid_new, saturation_new, vp_new, vs_new, rho_new);
}

void 
FluidSubstitution::DoGassmannReuss(const Rock                         * rock, 
                                   const std::vector< const Fluid * >   fluid_old, 
                                   const std::vector< double >          saturation_old, 
                                   const double                         vp_old, 
                                   const double                         vs_old, 
                                   ...
                                   double                             & rho_new) const
{
  ...
  if (r1 != 0.0 && r2 != 0.0 && l2 != 0.0 && k_sat_new != -1.0){
    vp_new  = std::sqrt( (k_sat_new + c*g_sat_new) / rho_new);
    vs_new  = std::sqrt( g_sat_new / rho_new);
    rho_new = rho_old + poro*(rho_fluid_new - rho_fluid_old);
  }
  else{  // This situation is effectively equivalent to poro == 0.0, or to an incompressible saturated rock.
    vp_new  = vp_old;
    vs_new  = vs_old;
    rho_new = rho_old;
  }
