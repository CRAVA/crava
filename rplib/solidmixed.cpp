  ...
  if (solid.size() != volume_fraction.size())
    ...
    switch (mix_method_) {
      ...
      case DEMTools::Reuss :
        ...
        mu_   = DEMTools::CalcEffectiveElasticModuliUsingReuss(mu[0], volume_fraction[0], mu[1]);
        break;
      case DEMTools::Voigt :
        k_    = DEMTools::CalcEffectiveElasticModuliUsingVoigt(k[0], volume_fraction[0], k[1]);
        mu_   = DEMTools::CalcEffectiveElasticModuliUsingVoigt(mu[0], volume_fraction[0], mu[1]);
        break;
      default :
