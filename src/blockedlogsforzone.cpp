...
//BlockedLogsForZone::BlockedLogsForZone(const NRLib::Well    & well,
//                                       const StormContGrid  & stormgrid,
...
//  dz_      = static_cast<double>(stormgrid.GetLZ()/stormgrid.GetNK());
//
//  std::vector<int> bInd(well.GetNumberOfNonMissingData()); //GetNData->getNd()); // Gives which block each well log entry contributes to
//
//  findSizeAndBlockPointers(stormgrid, bInd, mapped_blocked_log, well.GetNumberOfNonMissingData());
//  findBlockIJK(stormgrid, bInd, mapped_blocked_log);
//
//  blockContinuousLog(bInd, &mapped_blocked_log->GetVpBlocked()[0], alpha_);
//  blockContinuousLog(bInd, &mapped_blocked_log->GetVpBlocked()[0], beta_);
//  blockContinuousLog(bInd, &mapped_blocked_log->GetRhoBlocked()[0], rho_);
//
//  blockContinuousLog(bInd, &mapped_blocked_log->GetVpBackgroundResolution()[0], alpha_highcut_background_);
//  blockContinuousLog(bInd, &mapped_blocked_log->GetVsBackgroundResolution()[0], beta_highcut_background_);
//  blockContinuousLog(bInd, &mapped_blocked_log->GetRhoBackgroundResolution()[0], rho_highcut_background_);
//}
...
//void
//BlockedLogsForZone::findBlockIJK(const StormContGrid    & stormgrid,
//                                 const std::vector<int>   bInd,
//                                 BlockedLogsCommon      * mapped_blocked_log)
//{
//  ipos_ = new int[nBlocks_];
...
//  //const double * y = well->getYpos(dummy);
//  //const double * z = well->getZpos(dummy);
//  const std::vector<double> & x = mapped_blocked_log->GetXpos();
//  const std::vector<double> & y = mapped_blocked_log->GetYpos();
//  const std::vector<double> & z = mapped_blocked_log->GetZpos();
//
//  //
//  // Set IJK for virtual part of well in upper part of stormgrid
//  //
...
//void
//BlockedLogsForZone::findSizeAndBlockPointers(const StormContGrid  & stormgrid,
//                                             std::vector<int>     & bInd,
//                                             BlockedLogsCommon    * mapped_blocked_log,
...
//  int nd = number_of_nonmissing_welldata;
//  const std::vector<double> & x = mapped_blocked_log->GetXpos();
//  const std::vector<double> & y = mapped_blocked_log->GetYpos();
//  const std::vector<double> & z = mapped_blocked_log->GetZpos();
//  //const double * x  = well->getXpos(dummy);
