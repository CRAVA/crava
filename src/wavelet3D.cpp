                              ...
                              std::vector<bool>            & hasWellData) const
{
  ...
  if (estimInterval.size() > 0) {
    const std::vector<double> xPos = blocked_log->GetXposRawLogs();
    const std::vector<double> yPos = blocked_log->GetYposRawLogs();
    const std::vector<double> zPos = blocked_log->GetZposRawLogs();
    for (int k=0; k<nz_; k++) {
