  ...
  if(error == 0)
  {
    ...
    if (areaParams != NULL)
    {
      ...
      if(error == 0)
      {
        ...
        if (timeSimbox->getdz() > 4.0f) { // Require this density for wavelet estimation or facies probabilities
          ...
          if((modelSettings->getOutputFlag() & (ModelSettings::FACIESPROB + ModelSettings::FACIESPROBRELATIVE > 0))) {
                    ...
                    char           * errText)
{
  ...
  else
  {
    ...
    {
      wells[i] = new WellData(wellFile[i], modelSettings, headerList, modelFile->getFaciesLogGiven());
