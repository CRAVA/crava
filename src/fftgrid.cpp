...
//void
//FFTGrid::fillInFromSegYNew(const SegY   * segy,
...
//
//          if(cubetype_ != DATA) {
//            //Add trend before interpolating (because of shift).
//            float trend_inc = (trend_last - trend_first) / (rmt - 1);
//            for(int k_trace = 0; k_trace < rmt; k_trace++) {
//              rAmpFine[k_trace] += trend_first + k_trace * trend_inc;
//            }
//          }
//
...
//void
//FFTGrid::fillInFromStormNew(const Simbox  * timeSimbox,
//                            StormContGrid * grid,
//                            float           smooth_length,
//                            int           & missingTracesSimbox,
//                            int           & missingTracesPadding,
...
//        }
//
//        float dz_data = (z_max- z_min) / grid->GetNK();
//        float dz_min = dz_data/4.0f;
//
//        z0_data = z_min;
//
...
//        //An alternative is to add trend before interpolating, but then other values then zero must be added if dl < l2 || d > n_fine
//        if(cubetype_ != DATA) {
//          std::vector<float> trend_interpolated(nzp_);
...
void
FFTGrid::interpolateAndShiftTrend(std::vector<float>       & interpolated_trend,
                                  float                      z0_grid,
                                  float                      dz_grid,
...
//int
//FFTGrid::fillInFromSegYOld(const SegY              * segy,
//                           const Simbox            * simbox,
...
//int
//FFTGrid::fillInFromStormOld(const Simbox      * actSimBox,
//                            StormContGrid     * grid,
//                            const std::string & parName,
//                            bool                scale,
//                            bool                nopadding)
